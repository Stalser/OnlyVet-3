...
export default function AccountPage() {
  ...
  const pets = useMemo(() => {
    ...
    myAppointments.forEach((a) => {
      ...
      if (!map.has(key)) map.set(key, { name: a.petName, species: a.species });
    });
  ...
  const myDocs = useMemo(() => {
    return mockMedicalDocs; // потом заменим на Supabase
  }, []);
        ...
        <section className="grid md:grid-cols-3 gap-4">
          ...
          <div className="md:col-span-2 rounded-2xl border bg-white p-4 space-y-2">
            ...
            <p className="text-[11px] text-gray-400">
              Позже данные будут подгружаться автоматически.
            </p>
          </div>

          {/* Питомцы */}
          <div className="rounded-2xl border bg-white p-4 space-y-2">
            <h2 className="font-semibold text-base">Питомцы</h2>

            {pets.length === 0 && (
              <p className="text-xs text-gray-500">
                Пока нет данных. После консультаций питомцы появятся здесь.
              </p>
            )}
          ...
          {myDocs.length > 0 && (
            <ul className="text-xs space-y-2">
              {myDocs.map((d) => (
                <li
                  key={d.id}
                  className="rounded-xl border p-3 bg-gray-50 flex justify-between"
                >
                  <div>
                  ...
                  <Link
                    href={`/account/appointment/${d.appointmentId}`}
                    className="text-blue-600 underline underline-offset-2 text-[11px]"
                  >
                    Открыть
                  </Link>
                </li>
